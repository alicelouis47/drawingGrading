...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'ImageNet':
        ...
        train_dataset = datasets.CIFAR10(args.data, train=True, transform=mec.loader.TwoCropsTransform(transforms.Compose(augmentation1),
...
def train(train_loader, model, optimizer, epoch, lr_schedule, momentum_schedule, lamda_schedule, args):
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, mec, mce],
    ...
    eigen_z1 = []
    eigen_z2 = []
    with open('eigenvalue_epoch_'+str(epoch)+'.txt', 'w') as ef:
        model.train()
        end = time.time()
        for i, (images, _) in enumerate(train_loader):
            ...
            # L1, V1 = torch.linalg.eig(z1 @ z1.T)
            # L2, V2 = torch.linalg.eig(z2 @ z2.T)
            # eigen_z1.append(L1.cpu().detach().numpy().tolist())
            # eigen_z2.append(L2.cpu().detach().numpy().tolist())
            ...
            # mce_loss = (mce_loss_func(p1, z2, correlation=args.correlation, logE=True) * 0.003
            #             + mce_loss_func(p2, z1, correlation=args.correlation, logE=True) * 0.003
            #             + mce_loss_func(p2, z1, correlation=args.correlation, logE=False)
            ...
            mce.update(MCE.item(), images[0].size(0))
            ...
            if i % args.print_freq == 0:
                progress.display(i)
        # eigen_data = {"z1" : eigen_z1, "z2": eigen_z2}
        # json_str = json.dumps(eigen_data)
        # ef.write(json_str)
    if args.wandb:
        wandb.log({"loss":losses.avg, "mec_loss":mec.avg, "mce_loss":mce.avg})
